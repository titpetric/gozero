...
// TestParser_BinOp covers the operator assignment: the right side of
// := or = goes through go/parser, folds its constant subtrees, and
// leaves exactly one of +, == or != to run, with a named rejection
...
func TestParser_BinOp(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"subtraction":         {`s := a - b;`, "operator - runs only in a constant expression"},
		"multiplication":      {`s := a * b;`, "operator * runs only in a constant expression"},
		"less than":           {`ok := a < b;`, "operator < runs only in a constant expression"},
		"logical and":         {`ok := a && b;`, "operator && runs only in a constant expression"},
