...
// outlive the defining run, and it inherits nothing from it. A return
// inside an if arm ends the body the way it ends a program, and any
// other error panics; only signatures without an error result reach
// this runner, so there is no result for one to travel in.
