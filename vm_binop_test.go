...
var binopPrograms = map[string]struct{ src, want string }{
	...
	"int8 negative compares": {
		`var n int8; n = -1; ok := n == -1; s := fmt.Sprintf("%v", ok); return s;`,
		func() string { n := int8(-1); ok := n == -1; return fmt.Sprintf("%v", ok) }(),
	},
	"int16 wraps": {
...
func TestBinopCompileErrors(t *testing.T) {
	...
	for name, tc := range map[string]struct{ src, want string }{
		...
		"nil operand": {
			`u := urlOf(); ok := u == nil;`,
			"nil cannot be an operand",
		},
		"composite operand": {
			`s := url.URL{} == url.URL{};`,
			"url.URL{...} cannot be an operand",
		},
		"stack name operand": {
			`s := outside + "x";`,
