...
func TestParseFor(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		if (f.cond != nil) != tc.cond || f.initName != tc.init || f.cmpOp != tc.op || f.postDec != tc.dec || len(f.body) != tc.body {
			t.Errorf("%s: cond=%v init=%q op=%q dec=%v body=%d, want %v %q %q %v %d",
				name, f.cond != nil, f.initName, f.cmpOp, f.postDec, len(f.body), tc.cond, tc.init, tc.op, tc.dec, tc.body)
		}
	}
}
...
func TestParseForErrors(t *testing.T) {
	for name, tc := range map[string]struct{ src, want string }{
		...
		"post other name":  {`for i := 0; i < 3; j++ { }`, "the post clause increments the loop variable i"},
...
func TestParseForBody(t *testing.T) {
	for name, src := range map[string]string{
		...
		"rangeInside": `for q.More() { for x := range xs { rec(x) } }`,
	} {
