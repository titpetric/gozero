...
// declareTypes builds prog's declared types into c.declaredTypes.
...
func (c *Compiler) declareTypes(prog *program) error {
	c.declaredTypes = make(map[string]reflect.Type, len(prog.types))
	...
	for i := range prog.types {
		...
		// roots carries the first segment of every binding, so the
		// shadow check is one map read rather than a scan of them.
		if c.roots[td.name] {
