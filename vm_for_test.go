...
func forRuntime(t testing.TB) (*Runtime, *[]string) {
	...
	bind := map[string]any{
		...
		// state lives in a value the program built, so both tiers of an
		// equivalence test start fresh.
		"budget": newForBudget,
		...
		// can move the loop variable without operator expressions.
		...
		"boom": func() (bool, error) { return false, errBoom },
		// wrap nests a struct two fields deep, which the direct tier's
		// single-field rule declines.
...
// condition must be a static bool, the operands static integers of
// one signedness, the literal adapts but cannot go negative against
// an unsigned operand, and a body cannot retype a header name.
func TestForCompileErrors(t *testing.T) {
	rt, _ := forRuntime(t)
	for name, tc := range map[string]struct{ src, want string }{
		"cond not bool":     {`for poke() { }`, "a loop condition must be a bool, not *url.URL"},
		"cond stack name":   {`for zz { poke() }`, "a loop condition from the stack has no static type"},
		"cond string name":  {`s := join("a"); for s { poke() }`, "a loop condition must be a bool, not string"},
		"init float":        {`for i := 1.5; i < 3; i++ { }`, "a loop variable's init must be an integer"},
		"init bool":         {`for i := true; i < 3; i++ { }`, "a loop variable's init must be an integer"},
		"init string call":  {`for i := join("a"); i < 3; i++ { }`, "a loop variable's init must be an integer, not string"},
		"init stack name":   {`for i := zz; i < 3; i++ { }`, "from the stack has no static type"},
		"operand slice":     {`parts := fields("a b"); for i := 0; i < parts; i++ { }`, "a comparison operand must be an integer, not []string"},
		"operand stack":     {`for i := 0; i < zz; i++ { }`, "from the stack has no static type"},
		"mixed signedness":  {`u := u8v(); n := count(); for i := 0; u < n; i++ { }`, "the operands' signedness must match"},
		"negative unsigned": {`u := u8v(); for i := u; u < -1; i++ { }`, "constant -1 cannot compare with the unsigned uint8"},
		"var retyped":       {`for i := 0; i < 3; i++ { i = poke() }`, "the loop variable i is reassigned as *url.URL inside the body"},
		"cond retyped":      {`b := mk(); ok := b.More(); for ok { ok = poke() }`, "the loop condition ok is reassigned as *url.URL inside the body"},
	} {
