...
func (c *Compiler) compileRecv(sc *cscope, a arg) (*vmRecv, error) {
	ch, t, err := c.chanSource(sc, *a.recv)
	if err != nil {
		return nil, err
	}
...
func (c *Compiler) compileSend(sc *cscope, s stmt) (*vmSend, error) {
	src := arg{kind: argVar, str: s.sendCh[0]}
	if len(s.sendCh) > 1 {
		src = arg{kind: argPath, path: s.sendCh}
	}
	ch, t, err := c.chanSource(sc, src)
	if err != nil {
		return nil, err
	}
	if t == nil || t.Kind() != reflect.Chan {
	...
	val, err := c.compileArg(sc, "send", 0, t.Elem(), *s.sendVal)
	if err != nil {
		return nil, err
	}
