...
// TestFoldedProgramZeroAlloc pins what a fold costs at run time:
// nothing. A constant expression is a literal by the time the
// compiler sees it, so a program of folds and one runtime operator
// runs on the direct tier and allocates nothing per run.
func TestFoldedProgramZeroAlloc(t *testing.T) {
	rt := NewRuntime()
	if err := rt.Bind("sink", func(bool) error { return nil }); err != nil {
		t.Fatal(err)
	}
	...
	`
	if err := rt.Supports(src); err != nil {
		t.Fatalf("a folded program should JIT: %v", err)
	}
	jp := jitProgramOf(t, rt, src)
	ctx := context.Background()
	if _, err := jp.run(ctx, nil, nil); err != nil {
	...
	if allocs != 0 {
		t.Fatalf("a folded program allocates %.2f per run, want 0", allocs)
	}
}
