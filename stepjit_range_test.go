...
// TestRangeMatchesReflect runs every range form through both tiers
// and requires identical results, effects and errors.
...
// ctx.Err() at the next iteration boundary.
...
func TestRangeSupports(t *testing.T) {
	rt, _ := rangeRuntime(t)
	if err := rt.Bind("cookies", func() []*url.URL {
		return []*url.URL{{Path: "/a"}, {Path: "/b"}}
	}); err != nil {
		t.Fatal(err)
	}
