...
// statement runs: + concatenates strings and adds integers and
// floats, == and != compare booleans, integers, floats and strings.
// The typing rule is one line of Go's: operands need identical static
// types, and a literal side adopts the named side's type, checked for
// representability at compile time. Constant subtrees never reach
// here: the parser folds them through go/constant (parser_goexpr.go),
// so an operand is a slot or one folded literal, and the both-literal
