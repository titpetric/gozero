...
// Type declarations: the grammar's first block form that is not a
// statement body. A declaration is braces around field lines, one
// field per line or per semicolon, no nesting beyond the braces:
//
...
// rejectBlockDecl names a declaration written inside a statement body.
// The sniff in Parse only runs at the program level, where a
// declaration belongs; without this the statement parser would read
// the same line as a call and report a missing '('.
func (p *Parser) rejectBlockDecl() error {
	save, saveNL := p.pos, p.nl
	decl := p.keyword("type") && p.ident() != "" && p.keyword("struct")
	p.pos, p.nl = save, saveNL
	if decl {
		return fmt.Errorf("parse: a type declaration cannot stand inside a body, declare the type at the program level (offset %d)", p.pos)
	}
	return nil
}
