...
// TestTypeDecl checks the declaration end to end on whatever tier
...
func TestTypeDecl(t *testing.T) {
	...
	for name, tc := range map[string]struct{ src, want string }{
		...
		"name list":         {"type Size struct {\n\tW, H int64\n}\ns := Size{W: 3, H: 4}; json.NewEncoder(dest).Encode(s);", "{\"W\":3,\"H\":4}\n"},
	} {
		rt, _ := typeRuntime(t)
...
// TestTypeDeclNameListBothTiers runs the name-list form, the surface
// go/parser admits that the hand parser did not, down both tiers: the
// expansion to one field per name has to be a parse-time detail the
// tiers cannot see.
func TestTypeDeclNameListBothTiers(t *testing.T) {
	const src = `
		type Size struct { W, H, Area int64 }
		s := Size{W: 3, H: 4, Area: 12};
		json.NewEncoder(dest).Encode(s.H);
...
func TestTypeDeclSupports(t *testing.T) {
	...
	for name, src := range map[string]string{
		...
		"name list":        "type Size struct { W, H int64 }\n" + `s := Size{W: 1}; json.NewEncoder(dest).Encode(s.W);`,
	} {
		rt, _ := typeRuntime(t)
...
// TestTypeDeclErrors pins the compile errors. The structural
// judgments are go/types' own words surfacing through the check in
// declareTypes; the reflect.StructOf ceilings and the registry misses
// keep this package's messages, because go/types cannot see either.
func TestTypeDeclErrors(t *testing.T) {
	for name, tc := range map[string]struct{ src, want string }{
		"redeclared":          {"type P struct { X int64 }\ntype P struct { Y int64 }\nreturn 1;", "P redeclared"},
		...
		"duplicate field":     {"type P struct {\n\tX int64\n\tX string\n}\nreturn 1;", "X redeclared"},
		"duplicate in list":   {"type P struct { X, X int64 }\nreturn 1;", "X redeclared"},
		...
		"unknown behind star": {"type P struct { X *Bogus }\nreturn 1;", `unknown field type "*Bogus"`},
...
// design doc records: structurally identical declarations return the
...
// BenchmarkTypeDeclCompile prices the stdlib front end: the declared
// case pays go/scanner, go/parser and a go/types check per compile,
// the host case compiles the same statements against a BindType'd
// shape. The difference is the whole cost of parsing declarations
// with the standard library, paid once per source string and
// amortized by the compile cache.
func BenchmarkTypeDeclCompile(b *testing.B) {
	type benchPoint struct {
		X int64
		Y int64
	}
	stmts := `p := Point{X: 3, Y: 4}; json.NewEncoder(dest).Encode(p.X);`
	for name, src := range map[string]string{
		"declared": "type Point struct {\n\tX int64\n\tY int64\n}\n" + stmts,
		"host":     stmts,
	} {
		rt := NewRuntime()
		...
		if name == "host" {
			if err := rt.BindType("Point", benchPoint{}); err != nil {
				b.Fatal(err)
			}
		}
